/*
  ...
  Pressing key 'l' the program enters in auto segmentation mode.
...
void draw()
{
  ...
  for(int y = 0; y < height ; y++)
  {
    for(int x =0; x < width ; x++)
    {
       ...
       if(bd.isMatch(x, y, i)) { pixels[x+y*width] = c ;} 
...
{ 
  ...
  for(int i = 0; i < bd.getBlobsNumber(); i++)
  { 
      setcolor = color(random(254), random(254), random(254));
  } 
 return setcolor;
}
...
//present(e.g. on the drawing's lines)at that position, a message is given. 
 void tab(){   
     ...
     text("no blob",x+5,y+13);fill(255);text("no blob",x+6,y+14);}
 ...
 {
   switch(key){ 
   case 'l': loop ^= true; loop();if(loop){println("AUTO\tpress l to return in manual mode\n\n");}
                                  else{println("MANUAL\tpress l to go in auto mode or" ); 
                                       println("\tclick the mouse to redraw the frame\n" );}
  }
 }
